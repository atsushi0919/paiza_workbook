...
このとき、頂点 s と頂点 t を端点とするパスを全て出力してください。
...
期待する出力
1 行目にパスの総数を出力してください。
その後改行して、頂点 s と t を端点とするパスを全て求め、
各パスが辿る頂点の番号を順番に左から半角スペース区切りでそれぞれ改行して出力してください。
2 行目以降に出力するパスの順番は問いません。
各パスは一番左が s であり、一番右が t となります。
全てのパスが W 個ならば、1 行目に W と出力し、その後 W 行出力してください。
