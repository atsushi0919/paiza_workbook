...
このとき、頂点 s と頂点 t を端点とし、s, t を通過点として含まないトレイルを全て出力してください。
...
期待する出力
1 行目にトレイルの総数を出力してください。その後改行して、頂点 s と t を端点とするトレイルを全て求め、
各トレイルが辿る頂点の番号を順番に左から半角スペース区切りでそれぞれ改行して出力してください。2 行目以降に出力するトレイルの順番は問いません。
各トレイルは一番左が s であり、一番右が t となり、その間には s と t は含まれません。
