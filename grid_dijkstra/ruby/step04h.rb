...
# https://paiza.jp/works/mondai/grid_dijkstra/grid_dijkstra__d4h
...
def solve(input_lines)
  ...
  # 最小コストの探索
  searched = Array.new(tickets + 1) { Array.new(h) { Array.new(w) } }
  ...
  while pq.size > 0
    ...
    # 探索済み
    #next if !searched[tickets][y][x].nil? && searched[tickets][y][x] <= tmp_cost
    next unless searched[tickets][y][x].nil?

    # スタートから現在位置までを確定
    searched[tickets][y][x] = tmp_cost
    ...
    vy.zip(vx).each do |dy, dx|
      ...
      # チケットを使い探索候補に追加
      pq.insert([ny, nx, tmp_cost, tickets - 1]) if 0 < tickets
    end
  end
