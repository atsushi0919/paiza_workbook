# グループを利用した OR の検索 (paizaランク C 相当)
# https://paiza.jp/works/mondai/regular_expression/regular_expression__expression_15
...
丸かっこ () で囲まれた文字列は、文字列のまとまりを表します。
例えば、(京都|東京)駅 という正規表現は、(京都|東京) の部分が 1 つのまとまりを表しており、京都駅 または 東京駅 にマッチします。
前回出てきたバーティカルバー | と組み合わせることで、さまざまな OR 検索を表現することができます。
