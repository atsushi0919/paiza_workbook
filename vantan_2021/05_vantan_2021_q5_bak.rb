...
r0 = Rectangle.new(0, 0, 70, 50, 1, 1)
r1 = Rectangle.new(0, 0, 100, 100, 1, 1)
...
canvas = []
if overlap?(r0, r2) > 0
  canvas << Rectangle.new(r0.x1, r0.y2, r2.x1, r2.y2, 0, 1)
  canvas << Rectangle.new(r0.x2, r0.y1, r2.x2, r2.y2, 0, 1)
end
if overlap?(r1, r3) > 0
  canvas << Rectangle.new(r1.x1, r3.y1, r3.x1, r1.y1, 0, 1)
  canvas << Rectangle.new(r1.x2, r3.y2, r3.x2, r1.y2, 0, 1)
end
if canvas.empty?
  canvas << Rectangle.new(0, 0, w, h)
end
=end

# 長方形の重なりを調べる
# rects = [r0, r1, r2, r3]
rects = [Rectangle.new(0, 0, w, h, 0, 1)]
t_rects = [Rectangle.new(0, 0, x_y[0][0], x_y[0][1], 1, 1),
           Rectangle.new(0, x_y[1][1], x_y[1][0], h, 1, 1),
           Rectangle.new(x_y[2][0], x_y[2][1], w, h, 1, 1),
           Rectangle.new(x_y[3][0], 0, w, x_y[3][1], 1, 1)]
z = 1
while true
...
byebug
# 色を塗る
rects.concat(canvas).sort_by! { |r| r.z }.reverse!
opes.each do |c, x, y|
  rects.each do |r|
