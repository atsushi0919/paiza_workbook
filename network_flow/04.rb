# 最大流問題 4 (paizaランク B 相当)
# https://paiza.jp/works/mondai/network_flow/network_flow__max_flow_4
...
=begin
前回は、s から t へのパス上に、フローが流れなくなるまで繰り返し流しましたが、この方法だと最大まで流れない場合があります。
そこで鍵となるのが逆辺です。

基本的な考え方は、フローが流れている辺には逆向きにフローを流し戻すことができる、ということです。

たとえば、辺 (a, b) の容量を 3 として、この辺に流量 3 のフローが流れているとしましょう。
このとき、この状態からさらに辺 (b, a) に、つまり逆向きに流量 3 のフローを流すことで、相殺して最初の何も流れていない状態に戻すことができます。
つまり、ある辺にフローを流すと、その辺の容量は流した分だけ減り、逆向きの辺の容量が流した分だけ増えるということを意味します。

この性質を考慮した上で繰り返しフローを流すことで、最大流を正しく求めることができます。

では、実際に逆辺を使ってフローを流す準備をしていきましょう。
